...
private extension CartViewController {
    func setupView() {
        ...
        tableView.registerClass(UITableViewCell.self)
            .registerClass(CartTableViewCell.self)
    ...
    func bindView() {
        typealias CellForRowAt = (UITableView, Int, CartViewCellViewModel) -> UITableViewCell
        let cellForRowAt: CellForRowAt = { tableView, row, model in
            guard let cell = tableView.dequeueReusableCell(CartTableViewCell.self, indexPath: .init(row: row, section: .zero)) else {
                return .init()
            }
        ...
        let tapCell = tableView.rx
            .modelSelected(CartViewCellViewModel.self)
            .asDriver()
