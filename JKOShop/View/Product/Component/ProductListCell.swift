...
extension ProductListCell {
    func configCell(_ viewModel: ShopItemsViewModel) {
        ...
        pictureImageView.kf.setImage(with: viewModel.image, placeholder: UIImage(systemName: "arrow.down.to.line"), options: nil, completionHandler: { [weak self] result in
            switch result {
            case .failure:
                self?.pictureImageView.image = UIImage(named: "warning")
            default:
                break
            }
        })
